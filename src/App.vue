...
<template>
    ...
    <v-navigation-drawer app :location="$vuetify.display.mobile ? 'bottom' : undefined" permanent :width="200">
      <v-list-item>
        ...
        <v-list-item-subtitle ><v-text-field
          ...
          id="id"
          variant="solo"
          v-model.number="selectedCampaign.coupons"
        ></v-text-field>
      ...
      <template v-slot:append>
        ...
        <v-btn class="" block color="error" @click="deleteCampaignOverlay = !deleteCampaignOverlay">Delete Campaign</v-btn>
      </template>
  ...
  <v-text-field v-if="campaigns.length === 0"
    contained="true"
    name="name"
    label="Name"
    placeholder="value"
    v-model="newCampaignName"
  ></v-text-field>
  <v-btn v-if="campaigns.length === 0" color="success" @click="newCampaign(); selectedCampaign = campaigns[0]">Create</v-btn>
</template>